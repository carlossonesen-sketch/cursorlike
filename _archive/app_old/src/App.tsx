...
export default function App() {
    ...
    try {
      ...
      const result = await runtimeStart(
        ...
        toolRootGlobal,
        {
          ...
          context_length: (typeof localSettings.context_length === "number" && localSettings.context_length > 0 ? localSettings.context_length : 32768),
        },
        null,
        null
      );
  ...
  const applyPendingEdit = useCallback(
    async () => {
      ...
      // NOTE: keep active proposal selected after Apply so editor stays open
      // Refresh editor to show applied content immediately (no reopen required)
      const idx = activeEntry?.pendingEdit?.selectedIndex ?? 0;
      const opened = pendingEdit.files[idx] ?? pendingEdit.files[0];
      if (opened) {
        try {
          const diskContent = await workspace.readFile(opened.path);
          setFileEditState({
          ...
          // Fallback: use proposed content if disk read fails for any reason
          setFileEditState({
            relativePath: opened.path,
            baselineText: opened.proposed,
            baselineUpdatedAt: Date.now(),
            originalText: opened.proposed,
            editedText: opened.proposed,
            dirty: false,
            lastSaveStatus: "idle",
            savedAt: Date.now(),
          });
